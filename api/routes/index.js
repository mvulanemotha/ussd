...
router.get("/", async (req, res) => {
  ...
  try {
    ...
    let dbText = "";
    let logged = 0; //check if a user is logged
        ...
        if (data.length > 0) {
          ...
          await customer.checkNewUser(contact).then(async (dt) => {
            ...
            if (dt.length > 0) {
              ...
              await customer
                .saveCustomers(account, username, 0, password)
                .then(async (saved) => {
                  if (saved["affectedRows"] === 1) {
                    ...
                    response =
                      "SCBS -:- <br>Please use the password sent to you to login.";
                    closeOropenSession = 0;
                  } else {
                    response =
                      "SCBS -:- <br>Please contact SCBS at +268 24171975 registration failed.";
                    closeOropenSession = 0;
                  }
                });
            } else {
    ...
    if (text !== "" && text.indexOf("*") === -1) {
      // when text only contains the paasword
    ...
    // testing if a user is logged 0 if not 1 if logged
    if (logged === 1 && newrequest === "0") {
      ...
      if (text.indexOf("2*3*2") !== -1 && text.length >= 7) {
        ...
        await account.getClientAccount(contact).then(async (data) => {
          ...
          await account.clientsProducts(customerCfi).then(async (dt) => {
            ...
            let amountThatWeCanBorrow =
              quickLoan.calculateQuickLoanAmount(interestPosted);

            //check if amount being borrowed is less than inputed amount

            if (
              parseFloat(amountBorrowed) > parseFloat(amountThatWeCanBorrow)
            ) {
              response = "SCBS -:-";
              response += "<br>";
              response +=
          ...
          if (dt.length === 0) {
            ...
            await account.getClientAccount(contact).then(async (data) => {
              ...
              await account.clientsProducts(customerCfi).then((dt) => {
                ...
                let interestPosted =
                  parseFloat(
                    (totalAmount * (1 + percent / 360) ** 30).toFixed(2)
                  ) - totalAmount;
            ...
            await account
              .getAccountSavingsAccountBalance(mulaAccountNo)
              .then((accBalance) => {
                mulaAccountBalance =
                  accBalance["data"]["summary"]["accountBalance"];
              });
      ...
      if (text === "2*1") {
        // get money from savings account to mobile money
      ...
      if (text.indexOf("2*1*") !== -1 && text.length >= 20) {
        ...
        //check amount to be disburse if its less than E5000.00

        if (!disbursment.maxDisbursment(amount)) {
          ...
          response =
            "SCBS -:-<br><br>You have reached our MoMo limit of E 5000.00 per transaction.";
          closeOropenSession = 0;
        }
      }
      ...
      if (text === "2*2") {
        // from momo account to savings account
      ...
      if (text.indexOf("1*") !== -1 && text.length === 3) {
        // view account deatails
        ...
        try {
          ...
          if (isLoan === 1) {
            await loans.prePayment(accountFound).then((dat) => {
              ...
              response +=
                "<br>Arreas Amount: E " + dat.data["summary"]["totalOverdue"];
              response +=
                "<br>Current Balance: E " +
                dat.data["foreClosureData"]["totalForeClosureAmount"];
              response +=
                "<br>Total Outstanding: E " +
                dat.data["summary"]["totalOutstanding"];
      ...
      if (data.length > 0) {
        ...
        await collections.paymentStatus(xxid, token).then(async (dt) => {
          ...
          if (status === "SUCCESSFUL") {
            ...
            await collections
              .makeDeposit(amount, accountNo, phone, time.myDate(newDate))
              .then(async (data) => {
                ...
                await sms
                  .smsCharge(smsAccount, time.myDate(newDate))
                  .then(async (paySms) => {
                    //pay sms charge
                    await disbursment
                      .payCharge(
                        smsAccount,
                        paySms["data"]["resourceId"],
                        0.95,
                        time.myDate(newDate)
                      )
                      .then((tt) => {
